...
export class StudentFeesService {
  ...
  async recordPayment(studentFeeId: number, amount: number, method: string, transactionId?: string) {
    const client = this.dbConfig.getDatabaseClient(this.request);
    const studentFee = await (client as any).studentFees.findUnique({
    ...
    const payment = await (client as any).feePayments.create({
      data: {
  ...
  async getPaidFeesBySchool(schoolId: number) {
    ...
    // 2️⃣ Get all students (single query)
    const students = await (client as any).student.findMany({
      where: { school_id, isRTE: false },
      select: { id: true, username: true, class_id: true }
    });
    ...
    for (const fee of allFees) {
      ...
      classMap[label].totalFeeAmount += Number(fee.total_amount || 0);
      classMap[label].feeIds.push(fee.id);

      classFeesMap[fee.class_id] = label;
    }

    // Count students per class
    for (const stu of students) {
      const label = classFeesMap[stu.class_id];
      if (label) classMap[label].totalStudents++;
    }
    ...
    // Calculate paid students & amounts
    for (const username in studentPaymentMap) {
      const payment = studentPaymentMap[username];
      const label = classFeesMap[payment.class_id];
      if (!label) continue;

      classMap[label].paidAmount += payment.paidAmount;

      const hasPaidAll =
        classMap[label].feeIds.length > 0 &&
        classMap[label].feeIds.every(id =>
          payment.paidFeeIds.includes(id)
        );

      if (hasPaidAll) {
        classMap[label].paidStudents++;
      }
    }
    ...
    for (const label in classMap) {
      const data = classMap[label];

      const pendingAmount =
        data.totalStudents * data.totalFeeAmount - data.paidAmount;

      const pendingStudent =
        data.totalStudents - data.paidStudents;

      classStudent[label] = data.totalStudents;
      PaidStudents[label] = data.paidStudents;
      pendingStudents[label] = pendingStudent;
      classPaidAmount[label] = data.paidAmount;
      classPendingAmount[label] = pendingAmount;

      totalClassStudent += data.totalStudents;
      totalPaidStudent += data.paidStudents;
      totalPendingStudent += pendingStudent;
  ...
  async getPendingFeeListClass(schoolId: number, class_id: number) {
    const client = this.dbConfig.getDatabaseClient(this.request);
    // 1️⃣ Find all classes in the school that have fee structures
    const totalStudents = await (client as any).student.findMany({
      where: {
        class_id,
        school_id: schoolId,
        isRTE: false,
